...
def display_progress_summary(results: List, total_time: float):
    ...
    successful = [r for r in results if hasattr(r, 'metadata') and r.metadata.total_speakers > 0]
    ...
    summary.add_row("Avg Time per File", f"{total_time/len(results):.1f}s" if results else "N/A")
    ...
    if successful:
        ...
        summary.add_row("Processing Speed", f"{total_duration/total_time:.1f}x realtime" if total_time > 0 else "N/A")
...
@app.command()
def analyze(
    source: List[str] = typer.Argument(..., help="Audio/video files or URLs to analyze"),
    ...
    compress: bool = typer.Option(True, "--compress/--no-compress", help="Compress output JSON files"),
    prettify: bool = typer.Option(True, "--prettify/--no-prettify", help="Format JSON output"),
    aws_instance: str = typer.Option("p3.2xlarge", "--aws-instance", help="AWS instance type for optimization"),
    enable_cloudwatch: bool = typer.Option(False, "--cloudwatch", help="Enable CloudWatch logging"),
    ...
    base_config, processing_config, validation_config, aws_config, speaker_config = configs
        ...
        with PipelineManager(
            ...
            if show_progress and len(valid_sources) > 1:
                ...
                with Progress(
                    ...
                    task = progress.add_task("Processing files...", total=len(valid_sources))
                    
                    # Process each file individually for better progress tracking
                    for i, src in enumerate(valid_sources):
                        progress.update(task, description=f"Processing {Path(str(src)).name}")
                        ...
                        try:
                            ...
                            # Export result
                            if compress and output_file.stat().st_size > 5 * 1024 * 1024:  # 5MB threshold
                                output_file = output_file.with_suffix('.json.gz')
                            
                            with open(output_file, 'w') as f:
                                f.write(result.model_dump_json(indent=2 if prettify else None))
                            ...
                            logger.error("file_processing_failed", source=str(src), error=str(e))
                            # Add empty result for failed processing
                            from src.models.output_models import create_empty_analysis_result
                            results.append(create_empty_analysis_result(Path(str(src)).name, 0.0))
    ...
    try:
        ...
        successful_results = [r for r in results if hasattr(r, 'timeline') and r.timeline]
...
def version():
    ...
    console.print("High-performance audio analysis pipeline for dynamic subtitle generation")
