...
class TestPipelineIntegration:
    ...
    def test_pipeline_manager_initialization(self, mock_cuda, base_config, processing_config, aws_config, speaker_config):
    ...
    @pytest.mark.asyncio
    async def test_single_file_pipeline_mock(self, base_config, processing_config, aws_config, speaker_config, sample_audio_file):
        ...
        with patch('torch.cuda.is_available', return_value=False), \
             ...
             patch.object(PipelineManager, '_run_speaker_diarization') as mock_diarization:
            ...
            from services.speaker_diarizer import DiarizationResult, SpeakerSegment, SpeakerInfo
            segments = [
    ...
    @pytest.mark.asyncio
    async def test_batch_processing_mock(self, base_config, processing_config, aws_config, speaker_config, integration_test_files):
    ...
    def test_pipeline_progress_tracking(self, base_config, processing_config, aws_config, speaker_config):
    ...
    def test_resource_monitoring_integration(self, base_config, processing_config, aws_config, speaker_config):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_integration(self, base_config, processing_config, aws_config, speaker_config, corrupt_audio_file):
    ...
    def test_pipeline_cleanup_integration(self, base_config, processing_config, aws_config, speaker_config, test_data_dir):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_processing_integration(self, base_config, processing_config, aws_config, speaker_config, integration_test_files):
        ...
        with patch('torch.cuda.is_available', return_value=False), \
            ...
            with PipelineManager(
                ...
                results = await pipeline.process_batch(
                    integration_test_files[:2],  # Process 2 files
                    max_concurrent=2
                )
                ...
                assert processing_time < 0.2 * len(integration_test_files[:2])  # Much faster than sequential
...
class TestEndToEndIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_pipeline_with_mocks(self, base_config, processing_config, aws_config, speaker_config, sample_audio_file, test_data_dir):
        ...
        with patch('torch.cuda.is_available', return_value=False), \
            ...
            with patch('pyannote.audio.Pipeline.from_pretrained') as mock_pipeline_class:
                ...
                class MockAnnotation:
                    def itertracks(self, yield_label=False):
                        ...
                        for i, (start, end) in enumerate(segments):
                            segment = type('obj', (object,), {'start': start, 'end': end})()
                            if yield_label:
    ...
    def test_configuration_integration(self, test_data_dir):
        ...
        assert validation_config.max_duration_seconds > validation_config.min_duration_seconds