...
class TestAudioExtractionDurationAccuracy:
    ...
    def test_audio_extraction_with_duration_validation(self, mock_validator_class, 
                                                     mock_sf_info, mock_convert, extractor):
    ...
    @patch.object(AudioExtractor, '_convert_to_wav_with_validation')
    @patch('src.services.audio_extractor.DurationValidator')
    def test_audio_extraction_duration_validation_failure(self, mock_validator_class, 
                                                         mock_convert, extractor):
...
class TestWhisperXDurationValidation:
    ...
    def test_whisperx_with_duration_validation(self, mock_load_diarization, 
                                             mock_load_alignment, mock_load_models, 
                                             mock_librosa, whisperx_pipeline):
        """Test WhisperX processing with duration validation"""
        # Mock audio loading
        import numpy as np
        ...
        mock_whisper_model.transcribe.return_value = {
            "segments": [
                ...
                {"start": 5.0, "end": 10.0, "text": "This is a test", "confidence": 0.8},
                {"start": 10.0, "end": 140.0, "text": "Long segment", "confidence": 0.85}
            ],
            "language": "en"
        }
        ...
        # Mock duration validator
        whisperx_pipeline.duration_validator.validate_extracted_audio = Mock(return_value={
        ...
        with patch('src.models.speech_recognizer.whisperx.align') as mock_align, \
             patch('src.models.speech_recognizer.whisperx.assign_word_speakers') as mock_assign:
...
class TestSubtitleTimelineValidation:
    ...
    def test_subtitle_timeline_coverage_validation_complete(self, subtitle_optimizer):
        ...
        optimized = subtitle_optimizer.optimize_segments_with_gap_filling(segments, 15.0)
        validation = subtitle_optimizer.validate_subtitle_timeline_coverage(optimized, 15.0)
    ...
    def test_subtitle_timeline_coverage_validation_with_gaps(self, subtitle_optimizer):
        ...
        optimized = subtitle_optimizer.optimize_segments_with_gap_filling(segments, 20.0)
        validation = subtitle_optimizer.validate_subtitle_timeline_coverage(optimized, 20.0)
        
        # Should detect gaps but coverage might still be reasonable
        assert validation["coverage_percentage"] < 100.0
        assert validation["gaps_detected"] is True or len(optimized) > 2  # Either gaps detected or filled
    ...
    def test_gap_filling_functionality(self, subtitle_optimizer):
        ...
        optimized_with_filling = subtitle_optimizer.optimize_segments_with_gap_filling(segments, 20.0)
        ...
        validation_with = subtitle_optimizer.validate_subtitle_timeline_coverage(optimized_with_filling, 20.0)
        validation_without = subtitle_optimizer.validate_subtitle_timeline_coverage(optimized_without_filling, 20.0)
        
        assert validation_with["coverage_percentage"] >= validation_without["coverage_percentage"]
...
class TestEndToEndDurationAccuracy:
    ...
    def test_pipeline_duration_accuracy_integration(self, mock_whisperx_class, 
        ...
        mock_whisperx_result = {
            "segments": [
                ...
                {"start": 5.0, "end": 140.0, "text": "Long segment", "speaker": "SPEAKER_01"},
            ],
            "language": "en",
            "duration_metadata": {
...
class TestDurationAccuracyBenchmarks:
    ...
    def test_duration_detection_performance(self):
        ...
        with patch.object(validator, '_probe_with_ffprobe', return_value=142.0), \
