...
class SimpleAWSClient:
    ...
    def __init__(
        self, bucket: str, region: str = "us-east-1", enable_metrics: bool = True
    ):
    ...
    def _get_instance_metadata(self) -> Dict[str, str]:
        ...
        try:
            ...
            resp = requests.get(
                "http://169.254.169.254/latest/meta-data/instance-type", timeout=0.5
            )
    ...
    def upload_results(
        self, results_dir: Path, prefix: str = "results/"
    ) -> List[S3Result]:
    ...
    def track_processing(self, duration: float, files: int, speakers: int):
