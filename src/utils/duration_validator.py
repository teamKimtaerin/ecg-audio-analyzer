...
class DurationValidator:
    ...
    def probe_video_duration(self, file_path: Union[str, Path]) -> DurationProbeResult:
        ...
        try:
            ...
            result.final_duration, result.confidence, result.method_used = self._determine_final_duration(result)
            
            # Validate consistency between methods
            self._validate_duration_consistency(result)
            
            self.logger.info("duration_probe_completed",
                           file=str(file_path),
            ...
            self.logger.error("duration_probe_failed", file=str(file_path), error=str(e))
    ...
    def _determine_final_duration(self, result: DurationProbeResult) -> tuple[Optional[float], float, str]:
        """
        ...
        for method, duration in valid_durations.items():
            matching_count = sum(
                1 for d in duration_values 
                if abs(d - duration) <= self.tolerance_seconds
            )
            matching_methods = [
                m for m, d in valid_durations.items()
                if abs(d - duration) <= self.tolerance_seconds
            ]
    ...
    def _validate_duration_consistency(self, result: DurationProbeResult):
        ...
        for duration in valid_durations:
            ...
            if difference > self.max_difference_seconds:
                ...
                self.logger.warning("duration_inconsistency", 
                                  final=result.final_duration,
                                  other=duration,
                                  difference=difference)
    
    def validate_extracted_audio(self, audio_path: Union[str, Path], expected_duration: float) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            percentage_diff = (difference / expected_duration) * 100 if expected_duration > 0 else 0
            ...
            if not is_valid:
                self.logger.warning("audio_duration_mismatch",
                                  expected=expected_duration,
                                  actual=actual_duration,
                                  difference=difference)
            else:
                self.logger.info("audio_duration_validated",
            ...
            self.logger.error("audio_validation_failed", file=str(audio_path), error=str(e))
            return {
                "valid": False,
                "error": str(e),
                "expected_duration": expected_duration,
    ...
    def detect_timeline_gaps(self, segments: List[Dict[str, Any]], total_duration: float) -> List[Dict[str, Any]]:
        """
...
def validate_audio_duration(audio_path: Union[str, Path], expected_duration: float) -> Dict[str, Any]:
