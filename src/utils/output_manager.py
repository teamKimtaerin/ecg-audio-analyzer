...
class OutputManager:
    ...
    def _ensure_output_directory(self):
        ...
        try:
            ...
            self.logger.info("output_directory_ready", 
                           path=str(self.base_output_dir))
        except Exception as e:
            self.logger.error("failed_to_create_output_directory", 
                            path=str(self.base_output_dir),
                            error=str(e))
            raise
    ...
    def save_json(self, 
        ...
        try:
            ...
            self.logger.info("json_file_saved", 
            ...
            self.logger.error("failed_to_save_json", 
                            path=str(output_path),
                            error=str(e))
            raise
    ...
    def save_analysis_result(self, 
                           result: Any,
                           input_filename: str,
                           custom_output_path: Optional[Union[str, Path]] = None) -> Path:
        """
    ...
    def load_json(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("failed_to_load_json", 
                            path=str(file_path),
                            error=str(e))
            raise
    ...
    def cleanup_old_files(self, 
                         max_age_days: int = 30,
                         pattern: str = "*.json",
                         subfolder: Optional[str] = None) -> int:
        """
        ...
        for file_path in files:
            try:
                ...
                self.logger.warning("failed_to_delete_old_file", 
                                  path=str(file_path),
                                  error=str(e))
    ...
    def get_output_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "writable": self.base_output_dir.exists() and os.access(self.base_output_dir, os.W_OK)
            }
...
def get_output_manager(base_output_dir: str = "output") -> OutputManager:
    ...
    if _global_output_manager is None or str(_global_output_manager.base_output_dir) != base_output_dir:
