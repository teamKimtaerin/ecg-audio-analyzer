...
class GPUOptimizer:
    ...
    def _apply_optimizations(self) -> None:
        ...
        if self.config.memory_fraction < 1.0:
            try:
                ...
                self.logger.info("memory_fraction_set", fraction=self.config.memory_fraction)
    ...
    def optimize_model(self, model: torch.nn.Module, model_name: str) -> torch.nn.Module:
        ...
        try:
            ...
            self.logger.info("model_moved_to_device", model_name=model_name, device=str(self.device))

            # 최신 GPU에서 half precision 활성화
            if (self.device.type == "cuda" and 
                self.config.enable_amp and
                torch.cuda.get_device_capability(0)[0] >= 7):  # Volta+
            ...
            self.logger.error("model_optimization_failed", model_name=model_name, error=str(e))
            return model
    ...
    def get_memory_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "usage_percent": round((allocated / total_mb) * 100, 1) if total_mb > 0 else 0
            }
...
def optimize_for_inference(model: torch.nn.Module, model_name: str, 
                         config: Optional[GPUConfig] = None) -> torch.nn.Module:
