...
class EmotionResult:
    ...
    def from_logits(cls, logits: torch.Tensor, labels: List[str]) -> "EmotionResult":
        ...
        return cls(emotion=emotion, confidence=confidence, probabilities=prob_dict)
...
class EmotionAnalyzer:
    ...
    def __init__(
        ...
        self.logger = get_logger().bind_context(
            service="emotion_analyzer", model=model_name, device=self.device
        )
    ...
    def _load_model(self):
        ...
        if self.tokenizer is None or self.model is None:
            ...
            self.tokenizer, self.model = self.model_manager.load_emotion_model(
                self.model_name
            )
            ...
            self.logger.info("emotion_model_loaded", labels=self.emotion_labels)
    ...
    def _analyze_from_acoustic_features(
        ...
        try:
            ...
            return EmotionResult(
                emotion=emotion, confidence=confidence, probabilities=probabilities
            )
    ...
    def analyze_text(self, text: str) -> EmotionResult:
        ...
        if not text or not text.strip():
            return EmotionResult(
                emotion="neutral", confidence=0.5, probabilities={"neutral": 1.0}
            )
    ...
    def analyze_audio_file(
        ...
        try:
            # Load audio segment
            y, sr = librosa.load(
                str(audio_path), sr=sample_rate, offset=start_time, duration=duration
            )
            ...
            self.logger.error(
                "audio_file_emotion_analysis_failed",
                file=str(audio_path),
    ...
    def batch_analyze_segments(
        ...
        """
        self.logger.info(
            "batch_emotion_analysis", file=str(audio_path), segments_count=len(segments)
        )
