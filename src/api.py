...
class AudioSegment:
    ...
    words: Optional[List[WordSegment]] = (
        None  # Word-level acoustic data for dynamic subtitles
    )
...
async def analyze_audio(
    ...
    logger = get_logger().bind_context(
        service="public_api", input_path=str(input_path), gpu_enabled=config.enable_gpu
    )
    ...
    try:
        ...
        if output_path:
            ...
            saved_path = output_manager.save_analysis_result(
                result, str(input_path), output_path
            )
        else:
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(
                asyncio.run, analyze_audio(input_path, config, output_path)
            )
...
async def _process_audio_with_real_models(
    input_path: Union[str, Path], config: AnalysisConfig, logger
) -> AnalysisResult:
    ...
    audio_extractor = AudioExtractor(
        target_sr=config.sample_rate, duration_tolerance=0.1
    )
            ...
            if end_sample > start_sample:
                ...
                try:
                    ...
                    volume_db = 20 * np.log10(
                        np.maximum(np.mean(np.abs(word_audio)), 1e-10)
                    )
    ...
    if config.emotion_detection and emotion_analyzer:
        ...
        try:
            ...
            logger.info(
                "emotion_analysis_completed", results_count=len(emotion_results)
            )
        except Exception as e:
        ...
        for seg in speaker_segments:
            ...
            try:
                ...
                try:
                    ...
                    features.mfcc_mean = [
                        float(np.mean(mfcc[i, :])) for i in range(mfcc.shape[0])
                    ]
                    features.pitch_mean = (
                        float(np.mean(pitch_values)) if len(pitch_values) > 0 else None
                    )
                    features.pitch_std = (
                        float(np.std(pitch_values)) if len(pitch_values) > 0 else None
                    )
                except:
    ...
    for i, speaker_seg in enumerate(speaker_segments):
        ...
        speaker_info = SpeakerInfo(
            speaker_id=speaker_seg.speaker_id, confidence=speaker_seg.confidence
        )
    ...
    avg_confidence = (
        sum(seg.speaker.confidence for seg in segments) / len(segments)
        if segments
        else 0
    )
    ...
    if config.emotion_detection and emotion_results:
        ...
        dominant_emotion = (
            Counter(emotions_list).most_common(1)[0][0] if emotions_list else None
        )
    ...
    for speaker_id in set(seg.speaker.speaker_id for seg in segments):
        speaker_segments = [
            seg for seg in segments if seg.speaker.speaker_id == speaker_id
        ]
        speakers_dict[speaker_id] = {
            "total_duration": sum(seg.duration for seg in speaker_segments),
            "segment_count": len(speaker_segments),
            "avg_confidence": sum(seg.speaker.confidence for seg in speaker_segments)
            / len(speaker_segments),
            "emotions": (
                list(
                    set(seg.emotion.emotion for seg in speaker_segments if seg.emotion)
                )
                if config.emotion_detection
                else []
            ),
        ...
        for segment in segments:
            if segment.words:
                for word in segment.words:
                    ...
                    if (
                        word.spectral_centroid is not None
                        and word.spectral_centroid > 0
                    ):
    ...
    result = AnalysisResult(
        ...
        metadata={
            ...
            "config": {
                ...
                "emotion_model": (
                    emotion_analyzer.config.emotion_classifier
                    if emotion_analyzer
                    else None
                ),
            },
