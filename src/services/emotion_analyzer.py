...
class AudioProcessor:
    ...
    def _resample(waveform: torch.Tensor, original_sr: int) -> torch.Tensor:
        ...
        resampler = torchaudio.transforms.Resample(
            original_sr, AudioProcessor.TARGET_SAMPLE_RATE
        )
...
class EmotionMetricsCalculator:
    ...
    def calculate(emotion_probs: Dict[str, float]) -> EmotionMetrics:
        ...
        valence, arousal = EmotionMetricsCalculator._calculate_valence_arousal(
            emotion_probs
        )
        intensity = EmotionMetricsCalculator._calculate_intensity(emotion_probs)

        return EmotionMetrics(valence=valence, arousal=arousal, intensity=intensity)
    ...
    def load(self):
        ...
        try:
            ...
            self.model = Wav2Vec2ForSequenceClassification.from_pretrained(
                self.config.model_name, num_labels=len(self.config.emotion_labels)
            )
...
class EmotionAnalyzer:
    ...
    def __init__(self, config: EmotionAnalysisConfig, device: Optional[str] = None):
        ...
        # Statistics
        self.stats = {"total_segments": 0, "total_time": 0.0}
    ...
    def analyze_segment(
        self, audio_path: Path, start_time: float, end_time: float, speaker_id: str
    ) -> EmotionAnalysis:
        """
        ...
        try:
            ...
            calibrated_confidence = self.confidence_calibrator.calibrate(
                raw_confidence, emotion_probs, self.config.confidence_threshold
            )
            ...
            self.logger.error(
                "emotion_analysis_failed", speaker_id=speaker_id, error=str(e)
            )
    ...
    def analyze_batch(
        ...
        for start_time, end_time, speaker_id in segments:
            result = self.analyze_segment(audio_path, start_time, end_time, speaker_id)
