...
class AccurateSpeakerDiarizer:
    ...
    def __init__(self, config: SpeakerDiarizationConfig, device: Optional[str] = None):
        ...
        self.logger.info(
            "accurate_diarizer_initialized", device=self.device, model=config.model_name
        )
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            self.pipeline = Pipeline.from_pretrained(
                self.config.model_name, use_auth_token=auth_token
            )
            ...
            self.pipeline.instantiate(
                {
                    ...
                    "segmentation": {"min_duration_off": 0.5817},
                }
            )
    ...
    def _extract_embeddings(
        ...
        if not self.embedding_model:
            ...
            for seg in segments:
                ...
                features.append(
                    [seg.start_time / 100, seg.duration / 10, seg.confidence]
                )
    ...
    def _process_chunk(
        self, waveform: torch.Tensor, start: float, end: float, sample_rate: int = 16000
    ) -> List[SpeakerSegment]:
        ...
        try:
            ...
            self.logger.warning(
                "chunk_processing_failed", start=start, end=end, error=str(e)
            )

        return segments

    def _advanced_clustering(
        self, segments: List[SpeakerSegment], embeddings: Optional[np.ndarray] = None
    ) -> List[SpeakerSegment]:
        ...
        if n_speakers > 1:
            clustering = AgglomerativeClustering(
                n_clusters=n_speakers, affinity="cosine", linkage="average"
            )
    ...
    def _estimate_num_speakers(self, features: np.ndarray, n_segments: int) -> int:
        ...
        for n in range(min_speakers, max_speakers + 1):
            ...
            clustering = AgglomerativeClustering(
                n_clusters=n, affinity="cosine", linkage="average"
            )
            labels = clustering.fit_predict(features)
                ...
                if mask.sum() > 0:
                    ...
                    if len(cluster_features) > 1:
                        cluster_dist = pairwise_distances(
                            cluster_features, metric="cosine"
                        )
    ...
    def _match_speakers_across_chunks(
        self, all_segments: List[SpeakerSegment]
    ) -> List[SpeakerSegment]:
                ...
                if prev_speakers and curr_speakers:
                    ...
                    for j, curr_spk in enumerate(curr_speakers):
                        for k, prev_spk in enumerate(prev_speakers):
                            ...
                            time_diff = abs(
                                curr_profiles[curr_spk]["avg_time"]
                    ...
                    for j, k in zip(row_ind, col_ind):
                        if similarity[j, k] > 0.3:  # Threshold for matching
                            ...
                            # New speaker
                            curr_spk = curr_speakers[j]
                            speaker_mapping[curr_spk] = (
                                f"SPEAKER_{global_speaker_id + 1:02d}"
                            )
                            global_speaker_id += 1
    ...
    def _renumber_speakers(
        ...
        speaker_map = {
            old_id: f"SPEAKER_{i+1:02d}" for i, old_id in enumerate(unique_speakers)
        }
    ...
    def _merge_segments(self, segments: List[SpeakerSegment]) -> List[SpeakerSegment]:
        ...
        for next_seg in segments[1:]:
            if (
                ...
                and next_seg.start_time - current.end_time <= self.merge_threshold
            ):
                # Merge segments
    ...
    def _compute_speaker_stats(
        ...
        speaker_data = defaultdict(
            lambda: {"duration": 0, "count": 0, "confidences": []}
        )
    ...
    def diarize_audio(
        ...
        try:
            ...
            if self.embedding_model and len(all_segments) < 1000:
                try:
                    embeddings = self._extract_embeddings(
                        waveform, all_segments, sample_rate
                    )
                except Exception as e:
            ...
            filtered_segments = [
                seg for seg in all_segments if seg.speaker_id in valid_speakers
            ]
