...
class FastAPIClient:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                async with aiohttp.ClientSession(
                    ...
                    self.logger.info(f"콜백 전송 시도 {attempt + 1}/{self.max_retries + 1} - Job ID: {payload.job_id}")
                    ...
                    async with session.post(
                        ...
                        if 200 <= response.status < 300:
                            self.logger.info(f"콜백 전송 성공 - Job ID: {payload.job_id}, Status: {response.status}")
                            return True
                            ...
                            # 서버 오류는 재시도
                            error_text = await response.text()
                            last_error = f"서버 오류 {response.status}: {error_text[:200]}"
                            self.logger.warning(
                ...
                self.logger.warning(f"콜백 타임아웃 - Job ID: {payload.job_id}, 시도: {attempt + 1}")
                
            except aiohttp.ClientConnectorError as e:
                last_error = f"연결 오류: {str(e)}"
                self.logger.warning(f"콜백 연결 오류 - Job ID: {payload.job_id}, 시도: {attempt + 1}, 오류: {str(e)}")
                
            except aiohttp.ServerDisconnectedError as e:
                last_error = f"서버 연결 끊김: {str(e)}"
                self.logger.warning(f"콜백 서버 연결 끊김 - Job ID: {payload.job_id}, 시도: {attempt + 1}")
                
            except aiohttp.ClientError as e:
                last_error = f"클라이언트 오류: {str(e)}"
                self.logger.warning(f"콜백 클라이언트 오류 - Job ID: {payload.job_id}, 시도: {attempt + 1}, 오류: {str(e)}")
                
            except Exception as e:
                last_error = f"예상치 못한 오류: {str(e)}"
                self.logger.error(f"콜백 전송 중 예상치 못한 오류 - Job ID: {payload.job_id}, 시도: {attempt + 1}, 오류: {str(e)}")
            
            # 마지막 시도가 아니면 지수 백오프로 대기
            if attempt < self.max_retries:
                wait_time = self.retry_delay * (2 ** attempt)
                self.logger.info(f"콜백 재시도 대기 - Job ID: {payload.job_id}, 대기 시간: {wait_time}초")
                await asyncio.sleep(wait_time)
        
        self.logger.error(f"콜백 전송 최종 실패 - Job ID: {payload.job_id}, 마지막 오류: {last_error}")
        return False
    
    async def send_progress_update(self, job_id: str, progress: float, message: str = None) -> bool:
        """진행 상황 업데이트 전송"""
    ...
    async def send_error(self, job_id: str, error_message: str, progress: float = 0.0) -> bool:
        """오류 상태 전송"""
        payload = CallbackPayload(
            job_id=job_id,
