...
async def _run_analysis(input_path: str, config: AnalysisConfig, output_path: Optional[str], verbose: bool):
    ...
    try:
        ...
        rprint(f"⚙️  GPU: [{'green' if config.enable_gpu else 'red'}]{'Enabled' if config.enable_gpu else 'Disabled'}[/{'green' if config.enable_gpu else 'red'}]")
        rprint(f"🎭 Emotions: [{'green' if config.emotion_detection else 'red'}]{'Enabled' if config.emotion_detection else 'Disabled'}[/{'green' if config.emotion_detection else 'red'}]")
...
def _display_results(result, verbose: bool):
    ...
    summary_table.add_row("Speed Ratio", f"{result.duration/result.processing_time:.1f}x")
    ...
    # Speaker breakdown
    if result.speakers:
        speaker_table = Table(title="👥 Speaker Breakdown", show_header=True, header_style="bold blue")
        ...
        for speaker_id, info in result.speakers.items():
            emotions = ", ".join(info.get("emotions", [])) if info.get("emotions") else "N/A"
    ...
    if verbose and result.segments:
        segments_table = Table(title="🎯 Segment Details", show_header=True, header_style="bold yellow")
...
def version():
    ...
    info_table.add_row("Supported Formats", ", ".join(VERSION_INFO["supported_formats"]))
    ...
    rprint(f"GPU Available: [{'green' if sys_info['gpu_available'] else 'red'}]{sys_info['gpu_available']}[/{'green' if sys_info['gpu_available'] else 'red'}]")
...
def test():
    ...
    try:
        ...
        if not info['gpu_available']:
            rprint(f"ℹ️  [yellow]Note: GPU not available, will use CPU mode[/yellow]")
